...
# ==============================================================================
# ENCODING RESULTS CACHING
# ==============================================================================

def save_encoding_results(parcel_bold, all_pca_results, all_encoding_results,
                          train_indices, test_indices, valid_mask, atlas,
                          filepath, pca_dims=None, subject=None, session=None, verbose=True):
    """
    Save encoding analysis results to disk to avoid recomputation.
    ...
    if verbose:
        print(f"\n{'='*80}")
        print("SAVING ENCODING RESULTS TO DISK")
        print(f"{'='*80}")
...
def load_encoding_results(filepath, verbose=True):
    """
    Load encoding analysis results from disk.
    ...
    if verbose:
        print(f"\n{'='*80}")
        print("LOADING ENCODING RESULTS FROM CACHE")
        print(f"{'='*80}")
...
def check_encoding_results_exist(filepath):
    """
    Check if encoding results file exists.
